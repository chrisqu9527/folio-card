...
把 FOLIO（X 提示词图鉴）收成一套可检索的数据库。每条风格有固定编号。别人只要报编号，就能复制那一套提示词。

编号在 2026-09-24 按图鉴当时的顺序冻结：**日期从新到旧，同日按 id**。`001` 不会因为以后新增而改成别的风格。新提示词只追加到末尾。
...
`no` 编号、`style` 风格名、`category` 分类、`title`、`prompt` 原文、`slots` 可替换槽位、作者与原帖、媒介（图像 / 视频）、标签、可选的模型备注和作者后记。
